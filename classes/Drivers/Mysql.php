...
class Drivers_Mysql extends Drivers_Driver
{
	...
	public function create_table($table_name, $fields, $primary_key = TRUE)
	{
		...
		{
			$primary_key = 'id';
			$fields = array_merge(array('id' => array('integer', 'null' => FALSE, 'unsigned' => TRUE)), $fields);
		}
	...
	{
		$this->run_query("SET foreign_key_checks = 0");
		...
		foreach ($params as $key => $param)
		{
			...
			if (is_string($key))
			{
				switch ($key)
				{
					...
					default: throw new Kohana_Exception('migrations.bad_column', array($key));
				}
